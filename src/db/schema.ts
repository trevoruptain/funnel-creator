...
// ── Meta Exports (single row, replace on upload) ─────────────────────
export const metaExports = pgTable("meta_exports", {
  id: uuid("id").defaultRandom().primaryKey(),
...
export const adImages = pgTable("ad_images", {
  id: uuid("id").defaultRandom().primaryKey(),
  adConceptId: uuid("ad_concept_id")
    .notNull()
    .references(() => adConcepts.id, { onDelete: "cascade" }),
...
export const responsesRelations = relations(responses, ({ one }) => ({
  ...
  funnelStep: one(funnelSteps, {
    fields: [responses.funnelStepId],
...
export const adImagesRelations = relations(adImages, ({ one }) => ({
  adConcept: one(adConcepts, {
    fields: [adImages.adConceptId],
    references: [adConcepts.id],
  }),
}));
